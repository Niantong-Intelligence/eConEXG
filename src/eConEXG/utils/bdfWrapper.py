...
class bdfSaver(EdfWriter, Thread):
    def __init__(self, filename, chs: dict, fs: int) -> None:
        self.fs = int(fs)
...
class bdfSaverEEG(bdfSaver):
    ...
    def write_chunk(self, frames: list):
        ...
        for frame in frames:
            ...
            self.__data_write[:, self.__save_cnt: min(self.__save_cnt + eeg_signal.shape[1], self.fs)] = eeg_signal
            self.__save_cnt += eeg_signal.shape[1]
            if self.__save_cnt >= self.fs:
                self.__data_q.put(np.vsplit(self.__data_write, self.chs_len))
                self.__save_cnt = 0
                self.__halt_flag.set()


class bdfSaverEEGIMU(bdfSaver):
    def __init__(self, filename, chs_eeg: dict, fs_eeg: int, chs_imu: dict, fs_imu: int, dev_type: str) -> None:
    ...
    def write_chunk(self, frames: list):
        ...
        for frame in frames:
            ...
            self.__data_write_eeg[:, self.__save_cnt_eeg: min(self.__save_cnt_eeg + eeg_signal.shape[1], self.fs_eeg)] = eeg_signal
            self.__data_write_imu[:, self.__save_cnt_imu] = imu_signal
            ...
            if self.__save_cnt_eeg >= self.fs_eeg or self.__save_cnt_imu >= self.fs_imu:
                self.__data_q.put(np.vsplit(self.__data_write_eeg, self.chs_eeg_len) + np.vsplit(self.__data_write_imu, self.chs_imu_len))
                self.__save_cnt_eeg = 0
                self.__save_cnt_imu = 0
                self.__halt_flag.set()
